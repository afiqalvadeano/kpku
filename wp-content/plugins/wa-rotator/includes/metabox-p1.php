...
function wpbc_contacts_form_page_handler()
{
    ...
    if ( isset($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
        ...
        if (isset($_REQUEST['id'])) {
            $item = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id = %s", $_REQUEST['id']), ARRAY_A);
            if (!$item) {
                $item = $default;
...
<tbody >
    ...
    <form >
		...
		<br>	
            <input id="name" name="name" type="text" placeholder="Name " value="<?php echo esc_attr($item['name'])?>"
                    required>
        </p>
		</div>	
